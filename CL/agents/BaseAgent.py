...
class BaseAgent(object):
    ...
    def _choose_device(self):
        ...
        if self.cuda:
            ...
            self.logger.info("User specified {num_gpus} GPUs: {gpu_devices}")
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch + 1, self.config.num_epochs + 1):
            ...
            if patience and self.iter_with_no_improv > patience:
                self.logger.info(
                    "accuracy not improved in {patience} epoches, stopped"
                )
                break
            # train
            self.current_epoch = epoch
            self.train_one_epoch()
            self.save_checkpoint_tsk_train
            # validate
    ...
    def cleanup(self):
        ...
        if hasattr(self, "best_val_epoch"):
            self.logger.info(
                "Best Val acc at {self.best_val_epoch}: {self.best_val_metric:.3}"
            )
        endtime = datetime.datetime.now()
        exe_time = endtime - self.starttime
        self.logger.info(
            "End at time: {endtime.strftime('%Y.%m.%d-%H:%M:%S')}, total time: {exe_time.seconds}s"
        )
    ...
    def copy_checkpoint(self, filename="checkpoint.pth.tar"):
        if (
            ...
            self.logger.info("Backup checkpoint_epoch_{self.current_epoch}.pth.tar")
            torchutils.copy_checkpoint(
                filename=filename,
                folder=self.config.checkpoint_dir,
                copyname="checkpoint_epoch_{self.current_epoch}.pth.tar",
            )


    def copy_ckeckpoint_tsk_train(self,filename="checkpoint_task_train.pth.tar" ):
        
        if (
            self.config.copy_checkpoint_freq
            #and self.current_epoch % self.config.copy_checkpoint_freq == 0   #fix it 
        
        ):
            self.logger.info(f"Backup checkpoint_epoch_{self.current_epoch}_task_{self.task_id}.pth.tar")
            torchutils.copy_checkpoint(
                filename=filename,
                folder=self.config.checkpoint_dir,
                copyname=f"train_checkpoint_epoch_{self.current_epoch}_task_{self.task_id}.pth.tar",
            )

    def copy_ckeckpoint_tsk_val(self,filename="checkpoint_task_val.pth.tar" ):
        ...
        if (
            ...
            torchutils.copy_checkpoint(
                filename=filename,
                folder=self.config.checkpoint_dir,
                copyname=f"val_checkpoint_epoch_{self.current_epoch}_task_{self.task_id}.pth.tar",
            )
