...
import torchvision
from pcs_cl.utils import reverse_domain, torchutils
...
class SSDALossModule(torch.nn.Module):
    def __init__(self, config, gpu_devices):
        ...
        self.k_task = config.loss_params.k_task
    ...
    def get_attr(self, domain, name):
        return getattr(self, f"{name}_{domain}")            #return getattr(self, name + "_" + domain)
#         return getattr(self, name + "_" + domain)
    
    def set_attr(self, domain, name, value):
        setattr(self, f"{name}_{domain}", value)              #setattr(self, name + "_" + domain, value)
#         setattr(self, name + "_" + domain, value)
    ...
    def set_broadcast(self, task_id, name, value, comm=False):
        #print("there")
        ...
        self.set_attr(task_id, f"{name}_broadcast", value)
#         self.set_attr(domain_name, f"{name}_broadcast", value)
        
        self.set_attr(task_id, name, None)
#         self.set_attr(domain_name, name, None)

        self.broadcast_name.append((task_id, name))
#         self.broadcast_name.append((domain_name, name))
    ...
    @torch.no_grad()
    def updated_new_data_memory(self, domain, task_id, indices, outputs):
        ...
        """
        if domain == "source":
            memory_bank_src_tsk = self.get_attr(task_id, "memory_bank_source_task")
            data_memory_src_tsk = torch.index_select(memory_bank_src_tsk, 0, indices)

            outputs = F.normalize(outputs, dim=1)
            m = self.m
            new_data_memory_src_tsk = data_memory_src_tsk * m + (1 - m) * outputs
            updated_new_data_tsk = F.normalize(new_data_memory_src_tsk, dim=1)
            
            
        if domain == "target":
            memory_bank_tgt_tsk = self.get_attr(task_id, "memory_bank_target_task")
            data_memory_tgt_tsk = torch.index_select(memory_bank_tgt_tsk, 0, indices)

            outputs = F.normalize(outputs, dim=1)
            m = self.m
            new_data_memory_tgt_tsk = data_memory_tgt_tsk * m + (1 - m) * outputs
            updated_new_data_tsk = F.normalize(new_data_memory_tgt_tsk, dim=1)
            
            
        
        return updated_new_data_tsk   #F.normalize(new_data_memory, dim=1)
    ...
    def _get_Z(self, domain, vec, t):
        ...
        """
        bank = self.get_attr(self.task_id, "memory_bank")  # [data_len]
    ...
    def _get_all_dot_products(self, domain, vec):
        ...
        bank = self.get_attr(self.task_id, "memory_bank")
    ...
    def _compute_I2C_loss(self, domain, loss_type, t=0.05):
        ...
        n_kmeans = len(k_list_task)

        cluster_centroids = self.get_attr(  #Fix it
            reverse_domain(domain), f"cluster_centroids_{clus}"
        )
        outputs = self.outputs

        for each_k_idx, k in enumerate(k_list_task):
    ...
    def _compute_proto_loss(self, domain, loss_type, t=0.05):
        ...
        cluster_labels = self.get_attr(domain, f"cluster_labels_{clus}") #Fix it
        cluster_centroids = self.get_attr(c_domain, f"cluster_centroids_{clus}")   #Fix it
        cluster_phi = self.get_attr(c_domain, f"cluster_phi_{clus}")  #Fix it
    ...
    def forward(self, task_id, indices, outputs, domain, gpu_idx):
        ...
        new_data_memory = self.updated_new_data_memory(domain,task_id, indices, outputs)
